...
public final class VarioHudElement implements HudElement {
	...
	public void render(GuiGraphics graphics, DeltaTracker deltaTracker) {
		...
		for (StatsPanel panel : StatsPanel.values()) {
			...
			if (panel.border()) graphics.renderOutline(at.x(), at.y(), boxWidth, boxHeight, BORDER);
		}
		if (chart) {
	...
	private void drawPanel(GuiGraphics graphics, Font font, StatsPanel panel,
	...
	private void drawVelocityMatrix(GuiGraphics graphics, Font font, StatsPanel panel,
			int x, int y) {
	...
	private void drawDistanceMatrix(GuiGraphics graphics, Font font, StatsPanel panel,
			int x, int y) {
	...
	private void drawTicksMatrix(GuiGraphics graphics, Font font, StatsPanel panel,
			int x, int y) {
		BounceTracker.Event touch = recorder.bounceTouch();
		BounceTracker.Event leave = recorder.bounceLeave();
		BounceTracker.Event deploy = recorder.bounceDeploy();
		...
		for (int column = 0; column < values.length; column++) {
			...
			graphics.drawString(font, value, matrix.left(column, font.width(value)), y, VALUE, true);
		}
	}
	 ...
	 */
	private MatrixLayout matrixHeading(GuiGraphics graphics, Font font, StatsPanel panel,
		...
		if (!label.isEmpty()) graphics.drawString(font, label, x + PAD, y, LABEL, true);
		for (int column = 0; column < headings.length; column++) {
			graphics.drawString(font, headings[column], matrix.left(column, font.width(headings[column])),
					y, MUTED, true);
		}
		return matrix;
	}
	...
	private int unitsRow(GuiGraphics graphics, Font font, int x, int y, String units) {
		graphics.drawString(font, units, x + PAD, y, LABEL, true);
		return y + LINE;
	}
	 ...
	 */
	private int columnRow(GuiGraphics graphics, Font font, StatsPanel panel,
		...
		graphics.drawString(font, value, matrix.left(column, font.width(value)), y, color, true);
		return y + LINE;
	}

	private int matrixRow(GuiGraphics graphics, Font font, MatrixLayout matrix,
	...
	private int distanceRow(GuiGraphics graphics, Font font, MatrixLayout matrix,
	...
	private int row(GuiGraphics graphics, Font font, StatsPanel panel, int x, int y,
			String label, String value, int color) {
		graphics.drawString(font, label, x + PAD, y, LABEL, true);
		graphics.drawString(font, value, x + panel.layoutWidth() - PAD - font.width(value), y, color, true);
		return y + LINE;
	}

	private int accelerationRow(GuiGraphics graphics, Font font, StatsPanel panel,
	...
	private int sinceApexRow(GuiGraphics graphics, Font font, StatsPanel panel,
		...
		graphics.drawString(font, absolute, matrix.left(0, font.width(absolute)), y, MUTED, true);
		graphics.drawString(font, delta, matrix.left(1, font.width(delta)), y,
	...
	private static void drawAccelerationArrow(GuiGraphics graphics, int chartOriginX,
	...
	private void drawAxisLabels(GuiGraphics graphics, Font font, int x, int y, int width, int height) {
		...
		graphics.drawString(font, fmt("%+.0f", VarioConfig.chartMinVy * TPS), x * 2 + 4, (y + height) * 2 - 24, MUTED, false);

		String minVxz = fmt("%.0f", VarioConfig.chartMinVxz * TPS);
		graphics.drawString(font, minVxz, x * 2 + 4, (y + height) * 2 - 12, MUTED, false);
		...
		if (VarioConfig.chartMinVxz < 0 && VarioConfig.chartMaxVxz > 0
			...
			graphics.drawString(font, origin, zero - font.width(origin) / 2, (y + height) * 2 - 12, MUTED, false);
		}
