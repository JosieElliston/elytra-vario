...
public final class VarioHudElement implements HudElement {
	...
	private int drawPanel(GuiGraphics graphics, Font font, Sample sample, int x, int y) {
		...
		if (VarioConfig.showPanelBorder) graphics.renderOutline(x, y, width, height, BORDER);
	...
	private int row(GuiGraphics graphics, Font font, int x, int y, String label, String value, int color) {
		...
		graphics.drawString(font, value, x + panelWidth() - PAD - font.width(value), y, color, true);
		return y + LINE;
	}

	private int accelerationRow(GuiGraphics graphics, Font font, int x, int y,
