...
final class MarkerPreviewController implements Controller<Integer> {
	...
	private static final class Element extends ControllerWidget<MarkerPreviewController> {
		...
		public void render(GuiGraphics graphics, int mouseX, int mouseY,
			...
			graphics.drawString(textRenderer, control.option().name(), x + 6, y + 5, 0xFFD7DADE);
			...
			try {
				...
				graphics.drawCenteredString(textRenderer,
		...
		private static void drawShadow(GuiGraphics graphics, LadderMarkerShape shape,
		...
		private static void drawMarker(GuiGraphics graphics, LadderMarkerShape shape,
				int outside, int centerX, int centerY, int top, int bottom,
				int clipLeft, int clipRight, int color) {
			int radius = shape.height() / 2;
			for (int row = -radius; row <= radius; row++) {
		...
		private static void fillClipped(GuiGraphics graphics, int left, int top,
		...
		private static void fillClippedExcluding(GuiGraphics graphics,
